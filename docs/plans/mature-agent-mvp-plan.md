...
- [ ] **MAM-05** Implement safe-boundary normal resume and stale-context replan. Progress: September 13, 2026 — `agent_session` reconstructs bounded provider-neutral fresh-turn context only when completed checkpoint and current workspace fingerprints match; unavailable/mismatched evidence fails closed to explicit stale-context/unavailable dispositions. Resume composition now retains the selected durable session ID, continues event sequences monotonically, injects only bounded completed context into a fresh normal-resume turn, and enables the stale replan gate on mismatch. A distinct terminal renderer/confirmation records digest-bound refreshed-plan acknowledgement without replacing command or patch approval. End-to-end terminal resume/replan orchestration and CLI workflow composition remain to complete this task.
- [ ] **MAM-06** Add terminal session-history, inspect, export, delete, and resume workflows. Progress: September 13, 2026 — CLI now uses explicit `agent start` and `agent sessions list|inspect|export|delete|resume` forms. Record operations are workspace-scoped; export/deletion use the durable record store; normal fingerprint-matched resume uses fresh-turn composition, while stale/unavailable resume exits fail closed without running a session. Git workspaces warn when `.fabrica/` is not explicitly ignored and Fabrica never edits `.gitignore`. Stale-context terminal replan orchestration remains coupled to the unfinished MAM-05 flow.
