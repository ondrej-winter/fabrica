...
- [x] **MAM-05** Implement safe-boundary normal resume and stale-context replan. Evidence: September 13, 2026 — normal resume reconstructs bounded provider-neutral context in a fresh model turn only when completed checkpoint and workspace fingerprints match. Mismatched or unavailable fingerprints enter a distinct terminal stale-context flow: an inspection-only planning turn must complete fresh `read_files` or `search_codebase` inspection, its displayed refreshed plan receives a digest-bound acknowledgement, and only then does a separate fresh execution turn begin. The acknowledgement remains distinct from and cannot replace command or patch approval.
- [x] **MAM-06** Add terminal session-history, inspect, export, delete, and resume workflows. Evidence: September 13, 2026 — CLI exposes `agent start` and `agent sessions list|inspect|export|delete|resume`; record operations remain workspace-scoped and export/deletion use the durable record store. Normal fingerprint-matched resume uses fresh-turn composition, while stale resume runs the inspection, displayed-plan acknowledgement, and separate execution flow. Git workspaces warn when `.fabrica/` is not explicitly ignored, non-Git workspaces do not warn, and Fabrica never edits `.gitignore`.
- [x] **MAM-07** Add deterministic reporting-only evaluation corpus. Evidence: September 13, 2026 — the agent_evaluation slice provides six versioned normalized-evidence fixtures, deterministic rule-based assertions, stable JSON reports, and the offline credential-free fabrica agent-evaluate command; it is reporting-only and not a release gate.
- [x] **MAM-08** Document the sensitive-data and policy-only isolation posture. Evidence: September 13, 2026 — README.md and docs/mature-agent-safety.md document local-record sensitivity, capture exclusions, .fabricaignore trade-offs, resume/replan boundaries, offline evaluation, and the policy-only non-sandbox isolation threat model.
- [x] **MAM-09** Complete full validation and handoff. Evidence: September 13, 2026 — the implementation commit records successful focused coverage and an offline quality gate; this checkout is revalidated below.
